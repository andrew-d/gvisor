...
type ndpRouterEvent struct {
	nicid tcpip.NICID
	addr  tcpip.Address
	// true if router was discovereted, false if invalidated.
	discovered bool
}
...
type ndpPrefixEvent struct {
	nicid  tcpip.NICID
	prefix tcpip.Subnet
	// true if prefix was discovereted, false if invalidated.
	discovered bool
}
...
// Router Lifetime.
func raBufWithOpts(ip tcpip.Address, rl uint16, optSer header.NDPOptionsSerializer) buffer.Prependable {
...
// Router Lifetime.
func raBufWithPI(ip tcpip.Address, rl uint16, prefix tcpip.AddressWithPrefix, onLink, auto bool, vl, pl uint32) buffer.Prependable {
	flags := uint8(0)
	if onLink {
		flags |= 128
	}
...
func TestRouterDiscoveryDispatcherNoRemember(t *testing.T) {
	...
	e.Inject(header.IPv6ProtocolNumber, raBuf(llAddr2, uint16(lifetime)).View().ToVectorisedView())
	select {
	...
	if got := s.GetRouteTable(); !reflect.DeepEqual(got, routeTable) {
...
func TestRouterDiscovery(t *testing.T) {
	...
	e.Inject(header.IPv6ProtocolNumber, raBuf(llAddr3, uint16(l3Lifetime)).View().ToVectorisedView())
	...
	e.Inject(header.IPv6ProtocolNumber, raBuf(llAddr2, uint16(l2Lifetime)).View().ToVectorisedView())
	select {
	case <-ndpDisp.routerC:
...
func TestNoPrefixDiscovery(t *testing.T) {
	...
	for i := 0; i < 7; i++ {
		...
		t.Run(fmt.Sprintf("HandleRAs(%t), DiscoverOnLinkPrefixes(%t), Forwarding(%t)", handle, discover, forwarding), func(t *testing.T) {
			...
			e.Inject(header.IPv6ProtocolNumber, raBufWithPI(llAddr2, 0, prefix, true, false, 10, 0).View().ToVectorisedView())
...
func TestPrefixDiscoveryDispatcherNoRemember(t *testing.T) {
	...
	const lifetime = 1
	e.Inject(header.IPv6ProtocolNumber, raBufWithPI(llAddr2, 0, prefix, true, false, lifetime, 0).View().ToVectorisedView())
	select {
...
func TestPrefixDiscovery(t *testing.T) {
	...
	e.Inject(header.IPv6ProtocolNumber, raBufWithPI(llAddr2, 0, prefix1, true, false, 0, 0).View().ToVectorisedView())
	select {
	case <-ndpDisp.prefixC:
	...
	e.Inject(header.IPv6ProtocolNumber, raBufWithPI(llAddr2, 0, prefix1, true, false, 100, 0).View().ToVectorisedView())
	waitForEvent(subnet1, true, defaultTimeout)

	// Should have added a device route for subnet1 through the nic.
	if got, want := s.GetRouteTable(), []tcpip.Route{{subnet1, tcpip.Address([]byte(nil)), 1}}; !reflect.DeepEqual(got, want) {
		t.Fatalf("got GetRouteTable = %v, want = %v", got, want)
	}

	// Receive an RA with prefix2 in a PI.
	e.Inject(header.IPv6ProtocolNumber, raBufWithPI(llAddr2, 0, prefix2, true, false, 100, 0).View().ToVectorisedView())
	waitForEvent(subnet2, true, defaultTimeout)

	// Should have added a device route for subnet2 through the nic.
	if got, want := s.GetRouteTable(), []tcpip.Route{{subnet1, tcpip.Address([]byte(nil)), 1}, {subnet2, tcpip.Address([]byte(nil)), 1}}; !reflect.DeepEqual(got, want) {
		t.Fatalf("got GetRouteTable = %v, want = %v", got, want)
	}

	// Receive an RA with prefix3 in a PI.
	e.Inject(header.IPv6ProtocolNumber, raBufWithPI(llAddr2, 0, prefix3, true, false, 100, 0).View().ToVectorisedView())
	waitForEvent(subnet3, true, defaultTimeout)

	// Should have added a device route for subnet3 through the nic.
	if got, want := s.GetRouteTable(), []tcpip.Route{{subnet1, tcpip.Address([]byte(nil)), 1}, {subnet2, tcpip.Address([]byte(nil)), 1}, {subnet3, tcpip.Address([]byte(nil)), 1}}; !reflect.DeepEqual(got, want) {
		t.Fatalf("got GetRouteTable = %v, want = %v", got, want)
	}
	...
	if got, want := s.GetRouteTable(), []tcpip.Route{{subnet2, tcpip.Address([]byte(nil)), 1}, {subnet3, tcpip.Address([]byte(nil)), 1}}; !reflect.DeepEqual(got, want) {
		t.Fatalf("got GetRouteTable = %v, want = %v", got, want)
	}

	// Receive an RA with prefix2 in a PI with lesser lifetime.
	lifetime := uint32(2)
	e.Inject(header.IPv6ProtocolNumber, raBufWithPI(llAddr2, 0, prefix2, true, false, lifetime, 0).View().ToVectorisedView())
	select {
	case <-ndpDisp.prefixC:
	...
	if got, want := s.GetRouteTable(), []tcpip.Route{{subnet3, tcpip.Address([]byte(nil)), 1}}; !reflect.DeepEqual(got, want) {
		t.Fatalf("got GetRouteTable = %v, want = %v", got, want)
	}

	// Receive RA to invalidate prefix3.
	e.Inject(header.IPv6ProtocolNumber, raBufWithPI(llAddr2, 0, prefix3, true, false, 0, 0).View().ToVectorisedView())
...
func TestPrefixDiscoveryMaxOnLinkPrefixes(t *testing.T) {
	...
	e.Inject(header.IPv6ProtocolNumber, raBufWithOpts(llAddr1, 0, optSer).View().ToVectorisedView())
	for i := 0; i < stack.MaxDiscoveredOnLinkPrefixes+2; i++ {
		if i < stack.MaxDiscoveredOnLinkPrefixes {
			select {
			case r := <-ndpDisp.prefixC:
...
func TestNoAutoGenAddr(t *testing.T) {
	...
	for i := 0; i < 7; i++ {
		...
		t.Run(fmt.Sprintf("HandleRAs(%t), AutoGenAddr(%t), Forwarding(%t)", handle, autogen, forwarding), func(t *testing.T) {
			...
			// Rx an RA with prefix with non-zero lifetime.
			e.Inject(header.IPv6ProtocolNumber, raBufWithPI(llAddr2, 0, prefix, false, true, 10, 0).View().ToVectorisedView())
...
func TestAutoGenAddr(t *testing.T) {
	...
	// Receive an RA with prefix1 in an NDP Prefix Information option (PI)
	// with zero valid lifetime.
	e.Inject(header.IPv6ProtocolNumber, raBufWithPI(llAddr2, 0, prefix1, true, true, 0, 0).View().ToVectorisedView())
	select {
	case <-ndpDisp.autoGenAddrC:
	...
	// Receive an RA with prefix1 in an NDP Prefix Information option (PI)
	// with non-zero lifetime.
	e.Inject(header.IPv6ProtocolNumber, raBufWithPI(llAddr2, 0, prefix1, true, true, 100, 0).View().ToVectorisedView())
	...
	e.Inject(header.IPv6ProtocolNumber, raBufWithPI(llAddr2, 0, prefix1, true, true, 5, 6).View().ToVectorisedView())
	select {
	case <-ndpDisp.autoGenAddrC:
	...
	// Receive an RA with prefix2 in a PI.
	e.Inject(header.IPv6ProtocolNumber, raBufWithPI(llAddr2, 0, prefix2, true, true, 100, 0).View().ToVectorisedView())
	...
	e.Inject(header.IPv6ProtocolNumber, raBufWithPI(llAddr2, 0, prefix1, true, true, newMinVL, 0).View().ToVectorisedView())
	select {
	case <-ndpDisp.autoGenAddrC:
...
func TestAutoGenAddrValidLifetimeUpdates(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			e.Inject(header.IPv6ProtocolNumber, raBufWithPI(llAddr2, 0, prefix, true, true, test.ovl, 0).View().ToVectorisedView())
			select {
			case r := <-ndpDisp.autoGenAddrC:
				if r.nicid != 1 {
					t.Fatalf("got r.nicid = %d, want = 1", r.nicid)
			...
			e.Inject(header.IPv6ProtocolNumber, raBufWithPI(llAddr2, 0, prefix, true, true, test.nvl, 0).View().ToVectorisedView())
...
func TestAutoGenAddrRemoval(t *testing.T) {
	...
	const lifetime = 5
	e.Inject(header.IPv6ProtocolNumber, raBufWithPI(llAddr2, 0, prefix, true, true, lifetime, 0).View().ToVectorisedView())
	select {
	case r := <-ndpDisp.autoGenAddrC:
		if r.nicid != 1 {
			t.Fatalf("got r.nicid = %d, want = 1", r.nicid)
